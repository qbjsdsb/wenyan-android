...
android {
    ...
    defaultConfig {
        ...
        // v0.8.14：P0-3b 紧急修复 — catch 在 flatMapLatest 外层导致 flow 完成后 retry() 永久失效（Kotlin issue #3594），用户点击"重试"按钮无法恢复，必须杀进程重启 App。修复：catch 移到 flatMapLatest 内部仅作用于内部 flow，外层 _retryTrigger 保持活跃，retry() 递增后重新订阅数据流。同时 ApiConfigScreen Crossfade 改用 Triple(isLoading, error, isEmpty) 处理 error 状态，避免加载失败误显示"暂无 API 配置"。沙箱本地构建 release APK（19MB）供下载测试（GitHub 账单问题 CI 无法跑）。361 tests 全绿。
        versionName = "0.8.14"
