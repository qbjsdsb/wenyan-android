...
android {
    ...
    defaultConfig {
        ...
        // v0.9.5：关于与教程子路由。
        //   新增 AboutTutorialScreen.kt（430 行）— 7 章 GroupedCard 深度教程：
        //   1) 软件定位与核心理念（以真题为纲 / 以知识网络为本 / 以深度背诵为用）
        //   2) 功能模块导览（5 个顶级 Tab：知识点 / 真题 / 卡片 / 错题本 / 设置）
        //   3) FSRS-6 间隔重复算法（4 大公式 + 4 状态调度 + 4 档评分 + ClockGuard）
        //   4) 三档记忆机制（EXACT 0.95 / FRAMEWORK 0.90 / UNDERSTAND 0.85）
        //   5) AI 助手与 RAG 架构（RAG + 苏格拉底三阶段 + 多服务商 + Prompt Injection 防护）
        //   6) 使用指南与学习路径（6 步入门 + 三阶段节奏）
        //   7) 技术信息与致谢
        ...
        //   Icons.AutoMirrored.Filled.MenuBook 修复弃用警告
        //   agent-pr-review ✅ Ready to merge（0 blocker, 0 must-fix）
        //   PRR ✅ READY TO RELEASE（Exception E1 debug 签名 fallback，用户已接受）
        //   RBR ✅ PASS（Pinned inputs + hermetic build + artifact identity）
        versionName = "0.9.5"
