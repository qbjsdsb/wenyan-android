...
android {
    ...
    defaultConfig {
        ...
        // v0.9.6：关于与教程精简重构 + 代码卫生审计修复。
        //   AboutTutorialScreen.kt 重构：7 章 430 行 → 5 节 ~384 行
        //     - HeroCard（欢迎卡：定位 + 三大理念）
        //     - SectionQuickStart（快速上手：3 步入门）
        //     - SectionModules（功能模块：5 个 Tab 简介）
        //     - SectionPrinciples（学习原理：FSRS + 三档记忆，可折叠 ExpandableInfoItem）
        //     - SectionAbout（关于：技术栈 + 致谢 + 免责声明）
        //     - 默认视图简洁，深度原理用 AnimatedVisibility 包裹按需展开
        //     - 竖屏友好：MaxContentWidth.compact 限宽 + LazyColumn spacedBy
        //   代码卫生审计修复（4 项）：
        ...
        //   本地验证：:app:assembleDebug + testDebugUnitTest 全绿（403 tests, 0 failures）
        versionName = "0.9.6"
